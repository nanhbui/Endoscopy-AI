 ...
 * VLM (/live/explain) and fills in `report`. This panel just renders that list —
 * thumbnail + label + a compact LLM conclusion (or a spinner while explaining).
 */

import Box from '@mui/material/Box';
import Typography from '@mui/material/Typography';
import CircularProgress from '@mui/material/CircularProgress';
import { Image as ImageIcon, Sparkles, X, AlertCircle } from 'lucide-react';
...
export function LiveCapturesPanel({
  ...
  return (
    <Box sx={{ width: { xs: '100%', md: 320 }, flexShrink: 0, display: 'flex', flexDirection: 'column', borderRadius: '16px', border: '1px solid #E2EAE8', backgroundColor: '#fff', overflow: 'hidden' }}>
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', p: captures.length ? 1.25 : 0, display: 'flex', flexDirection: 'column', gap: 1.25, maxHeight: { xs: 360, md: 560 } }}>
        {captures.length === 0 ? (
          ...
          <Box key={c.id} sx={{ borderRadius: '12px', border: '1px solid #ECF1F0', overflow: 'hidden' }}>
